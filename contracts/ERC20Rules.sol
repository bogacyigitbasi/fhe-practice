...
contract ERC20Rules {
    ...
    constructor() {
        ...
        whitelistedWallets[
            address(0x133725C6461120439E85887C7639316CB27a2D9d)
        ] = 1;
        whitelistedWallets[
            address(0x4CaCeF78615AFecEf7eF182CfbD243195Fc90a29)
        ] = 2;
        ...
        country2CountryRestrictions = [
            createRestriction(countries["us"], countries["fr"])
        ];
    }
    ...
    function transfer(
        ...
        condition = TFHE.and(blackListOK, c2cOK);
    ...
    //     euint8 fromCountry = TFHE.asEuint8(
    //         erc20.getIdentifier(from, "country")
    //     );
    //     euint8 toCountry = TFHE.asEuint8(erc20.getIdentifier(to, "country"));
    //     require(
    //         TFHE.isInitialized(fromCountry) && TFHE.isInitialized(toCountry),
    ...
    function checkBlacklist(
        ...
        ebool fromBlacklisted = TFHE.asEbool(
            erc20.getIdentifier(from, "blacklist")
        );
        ebool toBlacklisted = TFHE.asEbool(
            erc20.getIdentifier(to, "blacklist")
        );
    ...
    function checkCountryToCountry(
        ...
        euint16 countryToCountry = TFHE.shl(TFHE.asEuint16(fromCountry), 8) +
            TFHE.asEuint16(toCountry);
        ...
        for (uint i = 0; i < c2cRestrictions.length; i++) {
            condition = TFHE.and(
                condition,
                TFHE.ne(countryToCountry, c2cRestrictions[i])
            );
        }
